...
const triggerLinguisticAnalysis = () => {
    const selectedText = englishAssistantStore.selectedText;
    if (!selectedText || selectedText.trim().length === 0) return;
    
    const request: TextLinguisticAssistantRequest = {
        text: selectedText,
        message: `analyze grammar for: ${selectedText}`
    };
    
    englishAssistantStore.streamEnglishTextLinguisticAssistant(request);
};
